...
class StackSphere(ManipulationEnv):
    """
    ...
    def _load_model(self):
        ...
        if self.placement_initializer is not None:
            ...
            self.placement_initializer.add_objects(self.cylinder)
            
        else:
            # breakpoint()
            # self.cube.set('pos','-0.2,0.1,0.8')
            # self.ball.set('pos','-0.2,0.1,0.8')
            # self.cylinder.set('pos','-0.2,0.1,0.8')
            
            z = 0.8
            existing_positions = []  # Keep track of positions to avoid conflicts

            # Generate positions for each cube
            # for cube in [self.cube,self.ball,self.cylinder]:
            ...
            self.placement_initializer = CustomUniformRandomSampler(
                name="ObjectSampler",
                mujoco_objects=[self.cube,self.sphere,self.cylinder],
                x_range=[-0.2, 0.2],
                y_range=[-0.2, 0.2],
                rotation=None,
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            sensors = [cube_pos, sphere_pos,cylinder_pos,
                       cube_quat,sphere_quat,cylinder_quat
                       ]
