...
from robosuite.environments.manipulation.stack_sphere import *
...
def get_object_pos_and_quats(env):
    obs = env.step([0,0,0,0,0,0,0])[0]
    # breakpoint()
    target_pos_cube = obs['cube_pos'] #+ np.array([0.0,0,0.1])
    obj_angle_cube = quat_to_euler(obs['cube_quat'])   / np.pi  * 180
    
    target_pos_sphere = obs['sphere_pos'] #+ np.array([0.0,0,0.1])
    obj_angle_sphere = quat_to_euler(obs['sphere_quat'])   / np.pi  * 180
...
#     obs = env.step([0,0,0,0,0,0,0])[0]
#     target_pos = obs['cube_pos'] #+ np.array([0.0,0,0.1])
#     obj_angle = quat_to_euler(obs['cube_quat'])   / np.pi  * 180
#     a0 = obj_angle[-1]
#     a1 = obj_angle[-1] + 90
#     a2 = obj_angle[-1] + 270
#     a3 = obj_angle[-1] + 180
...
if __name__ == "__main__":
    ...
    options["env_name"] = "StackSphere" # choose_environment()
