...
from robosuite.environments.manipulation.move_stacked_book import *
...
def get_object_pos_and_quats(env):
    ...
    target_pos_m = obs['cube_pos_m'] #+ np.array([0.0,0,0.1])
    obj_angle_m = quat_to_euler(obs['cube_quat_m'])   / np.pi  * 180
    ...
    return [
        ...
        ("box2",target_pos_m,obj_angle_m,np.array(env.cube_m.size)),
        ("box3",target_pos_l,obj_angle_l,np.array(env.cube_l.size))
    ]
    
    
def find_empty_spaces_random_position(all_objs, min_distance=0.2, range_limit=0.4):
    existing_positions = list(
        [
            [x[1][0],x[1][1]] for x in all_objs
        ]
    )
    while True:
        ...
        # breakpoint()
        all_away = [math.sqrt((x - ex)**2 + (y - ey)**2) >= min_distance for ex, ey in existing_positions]
        # print(all_away)
...
if __name__ == "__main__":
    ...
    options["env_name"] = "MoveStackBook" # choose_environment()
    ...
    # sort by z-idx
    all_objs = sorted(all_objs,key=lambda x:x[1][-1],reverse=True)
    obj_names = list([x[0] for x in all_objs])
    
    ########### Remove Top objects one by one
    for obj_name in obj_names[:-1]: # no need to move biggest /bottom obj
        _,target_pos,target_euler,curr_size = get_object_pos_and_quats_by_name(env,obj_name)
        
        target_euler = fix_eular(target_euler)
        z_suf = target_pos[2]
        
        target_x,target_y = find_empty_spaces_random_position(get_object_pos_and_quats(env))
        ...
        move_to_target_location(env, [target_pos,
                                    target_euler,
                                    ], position_tolerance=0.01, orientation_tolerance=5, max_steps=1000)
        grab_object(env,1)
        move_to_target_location(env, [target_pos+[0,0,0.1],
                                    target_euler,
                                    ], position_tolerance=0.01, orientation_tolerance=5, max_steps=1000,arm=1)
        # breakpoint()
        move_to_target_location(env, [np.array([target_x,target_y,target_pos[2]+0.1]),
                                    target_euler,
                                    ], position_tolerance=0.01, orientation_tolerance=5, max_steps=1000,arm=1)
        move_to_target_location(env, [np.array([target_x,target_y,target_pos[2]+0.1]),
                                    [0,0,0],
                                    ], position_tolerance=0.01, orientation_tolerance=5, max_steps=1000,arm=1)
        move_to_target_location(env, [[target_x,target_y,target_pos[2]-0.01],
                                    [0,0,0],
                                    ], position_tolerance=0.01, orientation_tolerance=5, max_steps=1000,arm=1,
                                break_on_z_force=1) # stop on z force 
        grab_object(env,-1)
        #zz = get_object_pos_and_quats(env)[obj_idx][1][-1]
        current_pos, current_quat = get_current_location(env)
        move_to_target_location(env, [[0,0,current_pos[-1]+0.1],
                                    [0,0,0],
                                    ], position_tolerance=0.12, orientation_tolerance=5, max_steps=1000,arm=0)


    ########### Standard Stack Ops
    ########## TODO: We need to pack these helpers and find a way to prompt GPT
    ...
    for obj_name in obj_names:
        ...
        # breakpoint()
        move_to_target_location(env, [np.array([0,0,target_pos[2]+0.1]),
                                    target_euler,
                                    ], position_tolerance=0.01, orientation_tolerance=5, max_steps=1000,arm=1)
        move_to_target_location(env, [np.array([0,0,target_pos[2]+0.1]),
                                    [0,0,0],
                                    ], position_tolerance=0.01, orientation_tolerance=5, max_steps=1000,arm=1)
        move_to_target_location(env, [[0,0,target_pos[2]-0.01],
                                    [0,0,0],
                                    ], position_tolerance=0.01, orientation_tolerance=5, max_steps=1000,arm=1,
                                break_on_z_force=1) # stop on z force 
        grab_object(env,-1)
    ...
    ########             END OF POLICY  ##########################
    idle(env,1000) # wait and see if things fall down
    print("SUCC? Common Sense ?",is_success(env))
    idle(env)
